> # ⚠️ RETRACTED — the results below are not real
>
> This is the **original** write-up, preserved unchanged as a historical record and as the subject of
> [`nb/01_leakage_forensics.ipynb`](../nb/01_leakage_forensics.ipynb).
>
> **Its headline results (R² = 0.83357, 89.74% directional accuracy) are an artifact of data
> leakage.** A Random Forest was being fitted to the test set's own labels inside `predict()`.
> Feeding the pipeline pure random noise instead of its LSTM outputs reproduces the same R².
> The model contributed nothing to the number.
>
> The honest result for daily S&P 500 returns is **R² ≈ 0** — they are not predictable.
> See the [current README](../README.md) for the corrected project.
>
> *Do not cite any number on this page.*
